...
var sitemap = require('../middleware/sitemap/sitemap.js')(require('../../contents/arbo.json').values);
...
module.exports = function (server, handler) {
	...
	server.use(sitemap.currentitem);
					...
					function(content, key, callback) {
						if (content.type == "libel") {
							if (content.storage == "inline") {
								...
								jf.readFile(path.join(server.get('dirname'), 'data', 'libels', content.source), function(err, data) {
							...
							if (content.storage == "inline") {
								...
								jf.readFile(path.join(server.get('dirname'), 'data', 'articles', content.source), function(err, data) {
									res.locals[key] = data.values;
									callback();
								});
							} else if (content.storage == "directory") {
	...
	for (var i = 0, n = sitemap.arbo.length; i < n; i++) {
