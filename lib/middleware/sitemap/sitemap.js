...
*/
	...
	var getBreadcrumb = function(indexes) {
		for (var i = 0, n = indexes.length, result = []; i < n; i++) {
			result[i] = {"tree": arbo[i].tree, "url": arbo[indexes[i]].fullroute, "anchor": arbo[indexes[i]].anchor, "title": arbo[indexes[i]].title, "subtitle": arbo[indexes[i]].subtitle, "active": (i == (n - 1)), "show": arbo[indexes[i]].show};
		};
		return result;
	};
...
//			if (arbo[i].tree.length == t.length && (function() {for (var k = 0, f = true; k < t.length - 1; k++) {f = (arbo[i].tree[j] == t[j])}; return f;})()) {
			if (sameFatherArray(arbo[i].tree, t)) {
				result[j] = {"tree": arbo[i].tree, "url": arbo[i].fullroute, "anchor": arbo[i].anchor, "title": arbo[i].title, "subtitle": arbo[i].subtitle, "active": (i == indexes[indexes.length - 1]), "show": arbo[i].show, "dropdown": arbo[i].dropdown, "subitems": []};
				if (arbo[i].dropdown) {
	...
	var getCurrentitem = function(indexes) {
		n = indexes.length - 1;
		return {"tree": arbo[n].tree, "url": arbo[indexes[n]].fullroute, "anchor": arbo[indexes[n]].anchor, "title": arbo[indexes[n]].title, "subtitle": arbo[indexes[n]].subtitle, "show": arbo[indexes[n]].show};
	};
	...
	var getHeaditems = function(indexes) {
		...
		for (var i = 0, j = 0, n = arbo.length; i < n; i++) {
			if ((arbo[i].tree.length == 2) && (t[0] == arbo[i].tree[0])) {
				result[j] = {"tree": arbo[i].tree, "url": arbo[i].fullroute, "anchor": arbo[i].anchor, "title": arbo[i].title, "subtitle": arbo[i].subtitle, "active": ((indexes.length < 2) ? false : (i == indexes[1])), "show": arbo[i].show};
				j++;
			};
		};
		return result.sort(function (a, b) {return compareArray(a.tree, b.tree);});
	...
	var getChilditemsfromtree = function(tree) {
		...
		for (var i = 0, j = 0, n = arbo.length; i < n; i++) {
			if ((arbo[i].tree.length == tree.length + 1) && beginsByArray(arbo[i].tree, tree)) {
				result[j] = {"tree": arbo[i].tree, "url": arbo[i].fullroute, "anchor": arbo[i].anchor, "title": arbo[i].title, "subtitle": arbo[i].subtitle, "active": false, "show": arbo[i].show};
				j++;
			};
		};
		return result.sort(function (a, b) {return compareArray(a.tree, b.tree);});
	...
	var getMainmenu = function(indexes) {
		...
		for (var i = 0, j = 0, n = arbo.length; i < n; i++) {
			if ((arbo[i].tree.length == 2) && (t[0] == arbo[i].tree[0])) {
				result[j] = {"tree": arbo[i].tree, "url": arbo[i].fullroute, "anchor": arbo[i].anchor, "title": arbo[i].title, "subtitle": arbo[i].subtitle, "active": ((indexes.length < 2) ? false : (i == indexes[1])), "show": arbo[i].show, "dropdown": arbo[i].dropdown, "subitems": []};
				if (arbo[i].dropdown) {
					result[j].subitems = getChilditemsfromtree(arbo[i].tree);
				};
				j++;
			...
			if ((arbo[i].tree.length == 1) && (t[0] == arbo[i].tree[0])) {
				result[j] = {"tree": arbo[i].tree, "url": arbo[i].fullroute, "anchor": arbo[i].anchor, "title": arbo[i].title, "subtitle": arbo[i].subtitle, "active": ((indexes.length == 1) ? (i == indexes[0]) : false), "show": arbo[i].show, "dropdown": false, "subitems": []};
				j++;
			};
		};
		return result.sort(function (a, b) {return compareArray(a.tree, b.tree);});
...
*/
	...
	return {
		...
		"currentitem" : (function () {
			return function(req, res, next) {
				res.locals.currentitem = getCurrentitem(routeIndexes(url2path(req.originalUrl)));
				next();
			};
		}) ()
		,
